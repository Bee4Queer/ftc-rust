            ...
            Ok(match name.as_str() {
                ...
                _ => {
                    return Err(Error::new(
                        name_ident.span(),
                        "ident should be one of linear, iterative, teleop, auto, utility, \
                         description, disabled, name, or group",
                    ));
                }
            })
        } else {
...
impl ToTokens for FtcArg {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        tokens.append(Ident::new(self.get_name(), self.get_span()));
        match self {
            Self::RenameCrate(ident, _) => {
                Token![=](Span::call_site()).to_tokens(tokens);
                ident.to_tokens(tokens);
            }
            Self::Name(text, _) | Self::Group(text, _) | Self::Description(text, _) => {
                Token![=](Span::call_site()).to_tokens(tokens);
...
/// - `linear`/`iterative` (exactly one is required) - Whether this is a linear op mode or an
///   iterative op mode.
...
/// - `rename_crate` - Takes the name of the ftc crate if it has been renamed. If not provided, uses
///   "ftc".
///
/// This functions by creating a java file under the new "autogenerated" directory within the
/// teamcode directory that contains
///
/// Examples:
///
/// ```no_run
...
///
/// use ftc::log::info; // the popular `log` crate is re-exported because of version hell
///
...
pub fn ftc(attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    if group.is_some() && utility {
        return Error::new(func.span(), "utility op modes cannot be in groups")
            .into_compile_error()
            .into();
    }
...
"#,
    ...
    if linear {
        let exported_func_name = format_ident!(
            "Java_org_firstinspires_ftc_teamcode_autogenerated_{class_name}_runOpMode"
        );
        quote! {
            #func
        ...
        let exported_init_loop_name = format_ident!(
            "Java_org_firstinspires_ftc_teamcode_autogenerated_{class_name}_init_1loop"
        );
